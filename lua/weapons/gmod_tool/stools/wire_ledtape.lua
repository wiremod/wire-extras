...
function TOOL:GetConVars()
	return self:GetClientInfo("material") or "cable/rope", self:GetClientNumber("width", 3)
end
...
if CLIENT then
	...
	for _, data in ipairs(TOOL.Information) do -- needed for wire extras, not sure why
		language.Add( "Tool.wire_ledtape." .. data.name, data.text )
	end
	...
	function TOOL:Preview()
		...
		local pt2 = Wire_LEDTape.DrawFullbright(self.CurWidth, self.ScrollMul / 3, color_white, self.CurMater, self.ToolPath)
