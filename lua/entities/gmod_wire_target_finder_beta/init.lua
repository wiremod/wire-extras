...
function ENT:SelectorNext(ch)
	if (self.Bogeys) and (#self.Bogeys > 0) then
		...
		if (self.SelectedTargets[ch]) and (self.SelectedTargets[ch]:IsValid()) then
            ...
            self:PlayerLock (self.SelectedTargets[ch], false)
            ...
            self:PlayerLock (self.SelectedTargets[ch], true)
	...
	if (self.Inputs.Hold) and (self.Inputs.Hold.Value > 0) then
		...
		for i = 1, self.MaxTargets do
			...
			if (!self.InRange[i]) or (!self.SelectedTargets[i]) or (self.SelectedTargets[i] == nil) or (!self.SelectedTargets[i]:IsValid()) then
                ...
                self:PlayerLock (self.SelectedTargets[i], false)
                if (#self.Bogeys > 0) then
                    ...
                    self:PlayerLock (self.SelectedTargets[i], true)
...
function ENT:PlayerLock (ent, targeted)
    if (IsEntity (ent) and ent:IsValid () and ent:IsPlayer ()) then
        if (targeted) then
            ent:TargetFinderLock (self)
        else
            ent:TargetFinderUnlock (self)
        end
    end
end
