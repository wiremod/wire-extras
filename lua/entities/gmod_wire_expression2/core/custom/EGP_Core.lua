--------------------------
--EGP E2 Base Functions!--
--------------------------

	--||+IGNORE+||--
--include("EGP/EGP_Shared.lua")
--AddCSLuaFile("EGP/EGP_Shared.lua")
--Loading the lib from here.
--I'm doing this for a reason cus then it can intergrate itself into E2 easyer.
--If this method fails for what ever reason i have an autorun that will boot the lib.
--but i should stop the EGP from working and causing problems.
--ill do that here.
...
e2function number wirelink:egpDraw()
	...
	for k,v in pairs(this.Render) do
		if not EGP.CacheCompare(this,k) then return end
		this:SendEntry(k, v)
	end
