...
TOOL.Category		= "Wire - Beta"    //"Wire - Beacon"
TOOL.Name			= "Target Finder"
TOOL.Command		= nil
TOOL.ConfigName		= ""

if ( CLIENT ) then
	language.Add( "Tool_wire_target_finder_beta_name", "Target Finder Beacon Tool (Wire)" )
	language.Add( "Tool_wire_target_finder_beta_desc", "Spawns a target finder beacon for use with the wire system." )
	language.Add( "Tool_wire_target_finder_beta_0", "Primary: Create/Update Target Finder Beacon" )
	...
	language.Add( "sboxlimit_wire_target_finder_betas", "You've hit target finder beacons limit!" )
	language.Add( "undone_wiretargetfinder", "Undone Wire Target Finder Beacon" )
end

if (SERVER) then
	CreateConVar('sbox_maxwire_target_finder_betas',30)
	CreateConVar("wire_target_finder_betas_maxtargets",10)
	CreateConVar("wire_target_finder_betas_maxbogeys",30)
end
...
cleanup.Register( "wire_target_finder_betas" )
...
function TOOL:LeftClick(trace)
	...
	if ( trace.Entity:IsValid() && trace.Entity:GetClass() == "gmod_wire_target_finder_beta" && trace.Entity.pl == ply ) then
	...
	if ( !self:GetSWEP():CheckLimit( "wire_target_finder_betas" ) ) then return false end
	
	local Ang = trace.HitNormal:Angle()
	
	local wire_target_finder_beta = MakeWireTargetFinderBeta( ply, trace.HitPos, Ang, range, players, npcs, npcname, beacons, hoverballs, thrusters, props, propmodel,  vehicles, playername, casesen, rpgs, painttarget, minrange, maxtargets, maxbogeys, notargetowner, entity, notownersstuff, steamname, colorcheck, colortarget, pcolR, pcolG, pcolB, pcolA, checkbuddylist, onbuddylist )
	
	local min = wire_target_finder_beta:OBBMins()
	wire_target_finder_beta:SetPos( trace.HitPos - trace.HitNormal*min.z )
	
	local const = WireLib.Weld(wire_target_finder_beta, trace.Entity, trace.PhysicsBone, true)
	
	undo.Create("WireTargetFinder")
		undo.AddEntity( wire_target_finder_beta )
		undo.SetPlayer( ply )
	undo.Finish()
	
	ply:AddCleanup( "wire_target_finder_betas", wire_target_finder_beta )
...
function TOOL:Reload(trace)
	if trace.Entity:IsValid() then
		self:GetOwner():ConCommand("wire_target_finder_beta_entityfil "..trace.Entity:GetClass().."\n")
	else
		self:GetOwner():ConCommand("wire_target_finder_beta_entityfil \n")
	end
	return true
end
...
if SERVER then
	...
	function MakeWireTargetFinderBeta(pl, Pos, Ang, range, players, npcs, npcname, beacons, hoverballs, thrusters, props, propmodel,  vehicles, playername, casesen, rpgs, painttarget, minrange, maxtargets, maxbogeys, notargetowner, entity, notownersstuff, steamname, colorcheck, colortarget, pcolR, pcolG, pcolB, pcolA, checkbuddylist, onbuddylist, Vel, aVel, frozen )
		if (!pl:CheckLimit("wire_target_finder_betas")) then return end
		
		local wire_target_finder_beta = ents.Create("gmod_wire_target_finder_beta")
		wire_target_finder_beta:SetPos(Pos)
		wire_target_finder_beta:SetAngles(Ang)
		wire_target_finder_beta:SetModel( Model("models/props_lab/powerbox02d.mdl") )
		wire_target_finder_beta:Spawn()
		wire_target_finder_beta:Activate()
		
		wire_target_finder_beta:Setup(range, players, npcs, npcname, beacons, hoverballs, thrusters, props, propmodel,  vehicles, playername, casesen, rpgs, painttarget, minrange, maxtargets, maxbogeys, notargetowner, entity, notownersstuff, steamname, colorcheck, colortarget, pcolR, pcolG, pcolB, pcolA, checkbuddylist, onbuddylist)
		wire_target_finder_beta:SetPlayer(pl)
		...
		table.Merge( wire_target_finder_beta:GetTable(), ttable )

		pl:AddCount( "wire_target_finder_betas", wire_target_finder_beta )
		
		return wire_target_finder_beta
	end

	duplicator.RegisterEntityClass("gmod_wire_target_finder_beta", MakeWireTargetFinderBeta, "Pos", "Ang", "range", "players", "npcs", "npcname", "beacons", "hoverballs", "thrusters", "props", "propmodel", "vehicles", "playername", "casesen", "rpgs", "painttarget", "minrange", "maxtargets", "maxbogeys", "notargetowner", "entity", "notownersstuff", "steamname", "colorcheck", "colortarget", "pcolR", "pcolG", "pcolB", "pcolA", "checkbuddylist", "onbuddylist","Vel", "aVel", "frozen")
...
function TOOL:UpdateGhostWireTargetFinder( ent, player )
	...
	if (!trace.Hit || trace.Entity:IsPlayer() || trace.Entity:GetClass() == "gmod_wire_target_finder_beta" ) then
		ent:SetNoDraw( true )
		return
	end
...
function TOOL.BuildCPanel(panel)
	panel:AddControl("Header", { Text = "#Tool_wire_target_finder_beta_name", Description = "#Tool_wire_target_finder_beta_desc" })
	...
	panel:AddControl("Slider", {
		...
		Min = "1",
		Max = "1000",
		Command = "wire_target_finder_beta_minrange"
	})
	...
	panel:AddControl("Slider", {
		...
		Min = "1",
		Max = "1000",
		Command = "wire_target_finder_beta_maxrange"
	})
	...
	panel:AddControl("Slider", {
		...
		Min = "1",
		Max = "10",
		Command = "wire_target_finder_beta_maxtargets"
	})
	...
	panel:AddControl("Slider", {
		...
		Min = "0",
		Max = "30",
		Command = "wire_target_finder_beta_maxbogeys"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_players",
		Command = "wire_target_finder_beta_players"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_notowner",
		Command = "wire_target_finder_beta_notargetowner"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_notownersstuff",
		Command = "wire_target_finder_beta_notownersstuff"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_npcs",
		Command = "wire_target_finder_beta_npcs"
	})
	
	panel:AddControl("TextBox", {
		Label = "#WireTargetFinderTool_npcname",
		Command = "wire_target_finder_beta_npcname",
		MaxLength = "20"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_beacons",
		Command = "wire_target_finder_beta_beacons"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_hoverballs",
		Command = "wire_target_finder_beta_hoverballs"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_thrusters",
		Command = "wire_target_finder_beta_thrusters"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_props",
		Command = "wire_target_finder_beta_props"
	})
	
	panel:AddControl("TextBox", {
		Label = "#WireTargetFinderTool_propmodel",
		Command = "wire_target_finder_beta_propmodel",
		MaxLength = "100"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_vehicles",
		Command = "wire_target_finder_beta_vehicles"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_rpgs",
		Command = "wire_target_finder_beta_rpgs"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_PaintTarget",
		Description = "#WireTargetFinderTool_PaintTarget_desc",
		Command = "wire_target_finder_beta_painttarget"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_casesen",
		Command = "wire_target_finder_beta_casesen"
	})
	
	panel:AddControl("TextBox", {
		Label = "#WireTargetFinderTool_playername",
		Command = "wire_target_finder_beta_playername",
		MaxLength = "50"
	})
	
	panel:AddControl("TextBox", {
		Label = "#WireTargetFinderTool_entity",
		Command = "wire_target_finder_beta_entityfil",
		MaxLength = "50"
	})
	
	panel:AddControl("TextBox", {
		Label = "#WireTargetFinderTool_steamname",
		Command = "wire_target_finder_beta_steamname",
		MaxLength = "50"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_colorcheck",
		Command = "wire_target_finder_beta_colorcheck"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_colortarget",
		Command = "wire_target_finder_beta_colortarget"
	})
	...
	panel:AddControl("Slider", {
		...
		Min = "0",
		Max = "255",
		Command = "wire_target_finder_beta_pcolR"
	})
	...
	panel:AddControl("Slider", {
		...
		Min = "0",
		Max = "255",
		Command = "wire_target_finder_beta_pcolG"
	})
	...
	panel:AddControl("Slider", {
		...
		Min = "0",
		Max = "255",
		Command = "wire_target_finder_beta_pcolB"
	})
	...
	panel:AddControl("Slider", {
		...
		Min = "0",
		Max = "255",
		Command = "wire_target_finder_beta_pcolA"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_checkbuddylist",
		Command = "wire_target_finder_beta_checkbuddylist"
	})
	
	panel:AddControl("CheckBox", {
		Label = "#WireTargetFinderTool_onbuddylist",
		Command = "wire_target_finder_beta_onbuddylist"
	})
