		...
		for k,v in pairs(KeyPos) do
			...
			if (tr.Entity == self.Entity) and (x >= 0) and (y >= 0) and (x <= 1) and (y <= 1) then
				...
				if (Ply:KeyDown(IN_USE)) and not (Ply.KeyOnce) then
					if (k <= 9) then
						Ply:ConCommand("gmod_keypadwire "..self.Entity:EntIndex().." "..k.." "..Rand.."\n")
					elseif (k == 10) then
						Ply:ConCommand("gmod_keypadwire "..self.Entity:EntIndex().." reset "..Rand.."\n")
					elseif (k == 11) then
						Ply:ConCommand("gmod_keypadwire "..self.Entity:EntIndex().." accept "..Rand.."\n")
					end
					Ply.KeyOnce = true
				end
	   		end
