...
AddCSLuaFile( "autorun/MHD_gates+chips.lua" )
...
local function AddMHDGates()
	GateActions["PHI"] = {
		...
		output = function(gate)
			return (2*math.cos(math.rad(36)))
		end,
		output = function(gate)
			return (-1*(math.sin(math.rad(666)) + math.cos(math.rad(6*6*6))))
		end	
	}
	//Q: What is Phi?
	//A: Phi is a irrational decimal value of approx. 1.6180339887...
	//   Phi resembles the ratio of the sides of an ordinay A4 paper sheet.
	...
	//   Phi resembles the expansion acceleration of the universe.
